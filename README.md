...
Use the GUI (if implemented in the notebook):
...
Loading Data: Reads healthcare-dataset-stroke-data.csv.

Handling NaN Values: Removes rows with missing bmi values using df.dropna().

Feature Selection: Drops id, work_type, Residence_type, and smoking_status columns.
...
Class Imbalance: Addresses the imbalance in the 'stroke' target variable using SMOTE to oversample the minority class.
